...
| **Encounter / location / provider (P0/P1)** | ✅ Done | Builder now **materialises real `Location` resources** (one per facility, parsed into name/address/phone) referenced from `Encounter.location`, **`Practitioner` resources** (deduped from CareTeam + report/referral/procedure performers) referenced from each, links **`DiagnosticReport`/`Observation`/`Procedure.encounter`** to the visit from the same C-CDA doc, and adds **`subject` → Patient** refs to all results. Verified: 124/124 encounter→location refs, 121/121 careteam→practitioner refs, 83 results→encounter, 249 subject refs all resolve. Plus the **Visits** screen and **Providers & locations** directory on the UI side. |
| **Capture dropped data (P1)** | ✅ Done | Builder now captures MyChart **letters** and the previously-dropped MyHealth **bloodPressure / vitalSigns / bloodOxygen / procedures** sections; no longer stamps undated procedures with the export timestamp; **dates each C-CDA document by its documented service/visit date** (not the shared export day) and titles it with that date + LOINC type + authoring institution (parsed from the CDA and `METADATA.XML`); and **promotes the surgical-consent HTML into a structured `Consent` + planned `Procedure`** linked to the source document. |
