...
*encrypted* `manifest.json` so they leak no information about the user's
data.
...
| `schema.version`  | yes      | Schema version of the *records inside*. See "Record shapes" below. |
...
// { encrypted: true, formatVersion: 1, appVersion?, createdAt? }
