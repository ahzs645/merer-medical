...
import { PageBackLinkProvider, usePageBackLink } from './pageBackLink';
 ...
 *
 * The side nav is drawn on the left but written second, and `lg:flex-row-reverse`
 * puts it back there — the same trick the app shell plays with its own rail. In
 * source order it was first, so reaching anything on a Records page took 20 tab
 * presses through the category list, on every one of the 33 routes that mount
 * this shell. Reversing the row costs nothing visually and hands the page's own
 * content the first tab stop.
 */
export function RecordsLayout() {
  return (
    <RecordCountsProvider>
      <PageBackLinkProvider>
        <RecordsShell />
      </PageBackLinkProvider>
    </RecordCountsProvider>
  );
...
function RecordsShell() {
  ...
  // Detail pages carry their own, more specific back link in the banner
  // ("All lab results"). Two stacked back links pointing at different places is
  // one too many, so this bar yields to theirs.
  const pageHasBackLink = usePageBackLink();

  return (
    <div className="flex h-full min-h-0 bg-gray-50 lg:flex-row-reverse">
